...
Bring the Phase 1 workspace-first foundation online with one canonical backend path, additive database changes, and the required deploy/runtime configuration for Supabase, Railway, and Evolution API.
...
- Backend deploy healthy on Railway.
- Frontend points to the Railway backend via `VITE_API_URL`.
